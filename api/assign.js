...
export default async function handler(req, res) {
  ...
  if (!bookingId || isNaN(parseInt(driverId))) {
  ...
  try {
    ...
    if (driverError || !driver) {
      console.warn(`Assignment blocked: Driver ${dId} not found.`);
    ...
    // 5. Assign booking with state check
    ...
    if (!booking) {
      console.warn(`Assignment conflict: Booking ${bookingId} already assigned or missing.`);
      return res.status(409).json({ success: false, error: "Conflict: Booking is no longer pending" });
    }

    console.log(`Assignment success: Booking ${bookingId} assigned to driver ${dId}`);
